...
class uiInfo:
    ...
    def getMoves(pokemon, game):
        ...
        tutorMoves = []
        cur.execute("""SELECT wp.name, wm.name, wm3.type_1, wm3.base_power, wm3.power_points, wm3.accuracy, wm3.priority,
                        wm3.damage_category, wm3.effect, wm3.description, wg1.name
                        FROM westwood_pokemon wp 
                        ...
                        WHERE wp.name = ? AND wg1.name IN (?) """, (pokemon,game))
                        #Selects move data using westwood_pokemon. The reason its a long call is that most of the data is linked via index tables - this is
                        #due to the data complexity.
        tutorMovesTemp = cur.fetchall()
        tutorMoves.append(tutorMovesTemp)
        cur.execute("""SELECT wp.name, wm.name, wm3.type_1, wm3.base_power, wm3.power_points, wm3.accuracy, 
                        wm3.priority, wm3.damage_category, wm3.effect, wm3.description, wg1.name
                        FROM westwood_pokemonform wp 
                        INNER JOIN westwood_pokemontutorsets wp1 ON ( wp.name = wp1.name  )  
                        INNER JOIN westwood_tutorsetslistelement wt ON ( wp1.tutor_sets = wt.list_id  )  
                        INNER JOIN westwood_tutorset wt1 ON ( wt.element_id = wt1.id  )  
                        INNER JOIN westwood_tutorsetmoveslistelement wt2 ON ( wt1.tutor_set_moves = wt2.list_id  )  
                        ...
                        INNER JOIN westwood_game wg1 ON ( wg.element_id = wg1.id  ) 
                        WHERE wp.name = ? AND wg1.name IN (?) """, (pokemon,game))
                        #Selects move data using westwood_pokemonforms. The reason its a long call is that most of the data is linked via index tables - this is
                        #due to the data complexity. This appends to the existing list.
